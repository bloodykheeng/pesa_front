...
const getChartOptions = (data, dataFilters) => {
    ...
    const options = {
        ...
        title: { text: `Customer Sales Performance <br/> ${chartTitle}` },
        xAxis: {
...
const CustomersBarChart = () => {
    ...
    console.log("testing CustomersBarChart such that it doesnt go into infinite due to useEffect dependancy array : ", filtersFormInitialDataValues);
    ...
    // Products
    const CustomerBarChartStatisticsPerfomanceChartQuery = useQuery({
        disable: false,
        queryKey: ["CustomerBarChartStatistics", ...Object.values(filtersFormInitialDataValues)],
        queryFn: () => getAllCustomerBarChartStatistics({ ...filtersFormInitialDataValues }),
    });

    // Use the custom hook to handle errors with useMemo on the error object
    useHandleQueryError(CustomerBarChartStatisticsPerfomanceChartQuery?.isError, CustomerBarChartStatisticsPerfomanceChartQuery?.error);

    console.log("data for NiceTwo CustomerBarChartStatisticsPerfomanceChartQuery ?.data?.data is:", CustomerBarChartStatisticsPerfomanceChartQuery?.data?.data);

    const seriesData = CustomerBarChartStatisticsPerfomanceChartQuery?.data?.data?.data;

    //=============== handle displaying filters on Chart end ======================

    return (
        <>
            {CustomerBarChartStatisticsPerfomanceChartQuery?.isLoading ? (
                <>
            ...
            ) : CustomerBarChartStatisticsPerfomanceChartQuery?.isError ? (
                <div style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <div style={{ maxWidth: "400px" }}>
