...
const CardsFiltersFormDialog = ({ onSubmit, filtersFormInitialDataValues, setFiltersFormInitialDataValues, showFiltersFormDialog, setShowFiltersFormDialog }) => {
    ...
    const [selectedProductCategoryBrand, setSelectedProductCategoryBrand] = useState(filtersFormInitialDataValues?.productCategoryBrand);
    ...
    useEffect(() => {
        ...
        setSelectedProductCategoryBrand(filtersFormInitialDataValues?.productCategoryBrand);
    ...
    const productsQuery = useQuery({
        queryKey: ["products", selectedProductCategoryBrand],
        queryFn: () => getAllProducts({ subCategories: selectedProductCategoryBrand }),
        enabled: selectedProductCategoryBrand.length > 0,
    });
    ...
    const handleReset = (resetForm) => {
        ...
        setSelectedProductCategoryBrand([]);
        resetForm();
                ...
                {({ values, setFieldValue, resetForm, errors, touched, isSubmitting }) => (
                    <Form>
                        <Grid container spacing={2} justifyContent="center" alignItems="center">
                                        ...
                                        {({ field }) => (
                                            <Autocomplete
                                                ...
                                                onChange={(event, newValue) => {
                                                    ...
                                                    setFieldValue("productCategoryBrand", []);
                            ...
                            <Grid item xs={12} md={6} lg={3}>
                                <Stack spacing={1}>
                                    <InputLabel htmlFor="productCategoryBrand">Product Category Brands</InputLabel>
                                    <Field name="productCategoryBrand">
                                        {({ field }) => (
                                            <Autocomplete
                                                //isOptionEqualToValue helps to define how comparison is gonna be made
                                                isOptionEqualToValue={(option, value) => option?.id === value?.id}
                                                ...
                                                onChange={(event, newValue) => {
                                                    setSelectedProductCategoryBrand(newValue);
                                                    setFieldValue("productCategoryBrand", newValue);
                                                    setFieldValue("products", []);
                                                }}
                                                renderInput={(params) => (
                                                    <TextField
                                                        {...params}
                                                        variant="outlined"
                                                        placeholder="Select product category brand"
                                    ...
                                    <ErrorMessage name="productCategoryBrand" component="div" style={{ color: "red" }} />
                                </Stack>
                            </Grid>
                            <Grid item xs={12} md={6} lg={3}>
                                <Stack spacing={1}>
...
export default CardsFiltersFormDialog;
